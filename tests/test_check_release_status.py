...
def test_rpm_built_not_released(workdir, config, rpm_dist):
    """
    Publish artifact exists but not in any repo; build artifact exists -> 'built, not released'.
    """
    component = _make_component(workdir, RPM_QUBESBUILDER)
    _write_fetch_artifact(config.artifacts_dir, component)
    # Publish artifact with no repository entries (package was signed but not published)
...
def test_windows_not_released_with_version_tag(workdir, config, win_dist):
    """
    Fetch done, no build artifact yet -> 'not released', version tag propagated.
    """
    component = _make_component(workdir, WIN_QUBESBUILDER)
    _write_fetch_artifact(config.artifacts_dir, component, vtags=["v1.0"])
    result = _check_release_status_for_component(
...
def test_windows_built(workdir, config, win_dist):
    """
    Build artifact present -> 'built'
    (Windows components are never 'released' to a repo).
    """
    component = _make_component(workdir, WIN_QUBESBUILDER)
    _write_fetch_artifact(config.artifacts_dir, component)
    _write_build_artifact(
...
def test_multiple_distributions(workdir, config, rpm_dist, win_dist):
    """
    A single component with both RPM and Windows targets is
    checked correctly for each distribution in one call.
    """
    mixed_qubesbuilder = """\
vm:
  rpm:
...
"""
    ...
    # Windows: built (build artifact exists)
    _write_build_artifact(
        config.artifacts_dir, component, win_dist, WIN_BUILD_MANGLE
    )
    ...
    assert result[component.name][win_dist.distribution]["status"] == "built"
