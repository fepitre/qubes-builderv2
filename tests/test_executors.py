...
from qubesbuilder.executors.qubes import LinuxQubesExecutor, WindowsQubesExecutor
...
def test_qubes_windows_simple():
    executor = WindowsQubesExecutor(os.environ.get("QUBES_EXECUTOR_DISPVM", "builder-dvm"))
    with tempfile.TemporaryDirectory() as temp_dir:
        # Create a local file with some content
        hello = Path(temp_dir) / "hello.md"
        with open(hello, "w") as f:
