...
class BaseWindowsExecutor(Executor, ABC):
    ...
    def _device_matches(dev, backend, loop_id: str) -> bool:
        ...
        dev_port = str(getattr(dev, "port_id", "") or "")
    ...
    def attach_ewdk(self, vm: str, vm_running: bool = False):
        ...
        timeout = 10
        while timeout > 0:
            for dev in backend.devices["block"].get_exposed_devices():
                if dev.port_id == loop_id:
                    assignment = (
                    ...
                    try:
                        if vm_running:
                            self.log.debug(f"hot-plugging EWDK to '{vm}'")
                            frontend.devices["block"].attach(assignment)
                        else:
                            self.log.debug(
                                f"assigning EWDK to '{vm}' (required, pre-start)"
                            )
...
class SSHWindowsExecutor(BaseWindowsExecutor):
    def __init__(
        ...
        **kwargs,
        ...
        if ewdk_mode == "attach" and ssh_vm is None and ewdk is not None:
            raise ExecutorError(
                "ewdk-mode 'attach' requires ssh-vm to be set: block-device attachment needs a Qubes VM name"
            )
    ...
    def start_worker(self):
        ...
        if self.ewdk_path and self.ewdk_mode == "attach" and vm_running:
            # VM was already running — hot-plug the EWDK
